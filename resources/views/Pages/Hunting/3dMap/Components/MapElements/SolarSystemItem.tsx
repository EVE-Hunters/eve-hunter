...
import { sphere } from './Resources/SphereGeomety'
import SystemContextMenu from './SystemContextMenu'

type MeshProps = JSX.IntrinsicElements['mesh']

interface SolarSystemItemProps extends MeshProps {
  system: SolarSystem
}

const SolarsystemSphere = React.forwardRef<
  Mesh<BufferGeometry>,
  SolarSystemItemProps
>(function (inProps, ref) {
  const { system, ...props } = inProps
  ...
  const [radius, setRadius] = useState<number>(0.07)

  const camera = useThree((state) => state.camera)

  let sphereColor = statistic <= 0 ? '#cacbcc' : color

  useFrame(() => {
    if (!isFocused) {
      return
    }
    const _r = isGrowing ? radius + 0.0007 : radius - 0.0007
    if (_r >= 0.08) {
      setShrinking()
    }
    if (_r <= 0.05) {
...
export default SolarsystemSphere
